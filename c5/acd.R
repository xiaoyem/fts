...
##
	...
	if (cond.dist == "exp") {
		...
		##
		fit = nlminb(start = params, objective = explk, lower = lowerB, upper = upperB)
	...
	if (cond.dist == "weibull") {
		...
		if (length(ini.est) == 0) {
			...
			if (s > 0) inibeta  = ini.est[(1 + r + 1):(1 + r + s)]
			params = c(omega = ini.est[1], alpha = inialpha, beta = inibeta,
				shape = ini.est[1 + r + s + 1])
		}
		##
		fit = nlminb(start = params, objective = weilk, lower = lowerB, upper = upperB)
		##lower = lowerB, upper = upperB, control = list(trace = 3))
		epsilon = 0.0001 * fit$par
		npar = length(params)
		Hessian = matrix(0, ncol = npar, nrow = npar)
	...
	if (cond.dist == "gamma") {
		...
		if (length(ini.est) == 0) {
			...
			if (s > 0) inibeta  = ini.est[(1 + r + 1):(1 + r + s)]
			params = c(omega = ini.est[1], alpha = inialpha, beta = inibeta,
				power = ini.est[2 + r + s], shape = ini.est[3 + r + s])
		}
		##
		fit = nlminb(start = params, objective = gamlk, lower = lowerB, upper = upperB)
		##lower = lowerB, upper = upperB, control = list(trace = 3))
		epsilon = 0.0001 * fit$par
		npar = length(params)
		Hessian = matrix(0, ncol = npar, nrow = npar)
